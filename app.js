...
var RV_CONFIG = {
  ...
  companyShort: '[Your Business]',
  supportEmail: 'demo@3120life.com',
  cashAppTag: '$YourBusiness',
...
function getListingsForClient(clientId) {
  var key = RV_KEYS.LISTINGS + (clientId || 'your-business');
  var data = rvGet(key);
...
function saveListingsForClient(clientId, arr) {
  rvSet(RV_KEYS.LISTINGS + (clientId || 'your-business'), arr);
}
...
function getClients() {
  ...
  if (!data || !Array.isArray(data)) {
    data = [
      { id: 'your-business', name: '[Your Business Name]', email: 'demo@3120life.com', plan: 'pro', role: 'pm', createdAt: '2026-01-15' },
    ];
...
// ============================================================================
var ACCOUNTS = [
  { email: 'demo@3120life.com', username: 'demo_user', password: '', name: '[Your Business]', role: 'admin', clientId: 'your-business' },
...
function confirmShowing(dateKey, time) {
  ...
  if (propData.address) {
    ...
    try {
      var listings = JSON.parse(localStorage.getItem('rv_listings_data') || localStorage.getItem('rv_listings_your-business') || '[]');
...
function openBroadcast(listingIndex, clientId) {
  var cid = clientId || 'your-business';
...
document.addEventListener('DOMContentLoaded', function() {
  getClients();
  getListingsForClient('your-business');
...
function generateStatementPDF(statement) {
  loadJsPDF(function(jsPDF) {
    ...
    doc.text('[Your Business Name]', 105, y, { align: 'center' }); y += 5;
    ...
    doc.text('Generated by 3120 Life — [Your Business Name]', 105, y, { align: 'center' });
...
function generateTaxPDF(summary) {
  loadJsPDF(function(jsPDF) {
    ...
    doc.setFontSize(10); doc.setFont('helvetica', 'normal');
    doc.text('[Your Business Name]', 105, y, { align: 'center' }); y += 10;
    doc.setDrawColor(200); doc.line(15, y, 195, y); y += 8;
...
function importSPMToClientHub(xlsxRows) {
  ...
  var listingsKey = RV_KEYS.LISTINGS + 'your-business';
